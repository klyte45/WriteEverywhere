    ...
    internal class BuildingInfoDetailLiteUI
    {
        ...
        private void RegularDraw(Vector2 size)
        {
            using (new GUILayout.VerticalScope())
            {
                ...
                using (new GUILayout.HorizontalScope(GUILayout.Height(80)))
                {
                    using (new GUILayout.VerticalScope(GUILayout.ExpandWidth(true)))
                    {
                        var skinNoWrap = new GUIStyle(GUI.skin.label) { wordWrap = false };
                        GUILayout.Label($"<color=#FFFF00>{Str.WTS_CURRENTSELECTION}</color>", skinNoWrap, GUILayout.MaxWidth(200));
                        ...
                        GUILayout.Label(m_currentSource.ValueToI18n(), skinNoWrap, GUILayout.MaxWidth(200));
                    ...
                    using (new GUILayout.VerticalScope())
                    {
                        using (new GUILayout.HorizontalScope())
                        {
                            if (CurrentLibState == FooterBarStatus.Normal)
                            {
                                ...
                                {
                                    GUIKwyttoCommons.SquareTextureButton2(m_createNew, Str.we_assetEditor_createLayoutAsset, CreateNew, !isEditable);
                                    GUIKwyttoCommons.SquareTextureButton2(m_deleteFromCity, Str.we_assetEditor_deleteFromAsset, DeleteFromCity, m_currentSource == ConfigurationSource.ASSET);
                                }
                                GUILayout.FlexibleSpace();
                                if (!SceneUtils.IsAssetEditor)
                                {
                                    ...
                                    GUIKwyttoCommons.SquareTextureButton2(m_exportAsset, Str.WTS_BUILDINGEDITOR_BUTTONROWACTION_EXPORTTOASSETFOLDER, ExportAsset, m_currentSource == ConfigurationSource.CITY && CurrentEditingInfo.name.EndsWith("_Data"));
                                }
                                else
                                {
                                    GUIKwyttoCommons.SquareTextureButton2(m_save, Str.WTS_BUILDINGEDITOR_BUTTONROWACTION_EXPORTTOASSETFOLDER, ExportAsset, m_currentSource == ConfigurationSource.ASSET);
                                }
                                GUILayout.FlexibleSpace();
