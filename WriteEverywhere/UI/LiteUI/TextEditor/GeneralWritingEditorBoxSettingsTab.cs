        ...
        public bool DrawArea(Vector2 tabAreaSize, ref BoardTextDescriptorGeneralXml currentItem, int currentItemIdx)
        {
            ...
            using (var scroll = new GUILayout.ScrollViewScope(m_scrollPos))
            {
                using (new GUILayout.VerticalScope())
                {
                    ...
                    if (usingFrame)
                    {
                        changedFrame |= GUIKwyttoCommons.AddVector2Field(tabAreaSize.x, item.BackgroundMeshSettings.FrameMeshSettings.BackSize, "K45_WTS_BOXMESH_BACKSIZE", "K45_WTS_BOXMESH_BACKSIZE", isEditable);
                        changedFrame |= GUIKwyttoCommons.AddVector2Field(tabAreaSize.x, item.BackgroundMeshSettings.FrameMeshSettings.BackOffset, "K45_WTS_BOXMESH_BACKOFFSETFROMCENTERBOTTOM", "K45_WTS_BOXMESH_BACKOFFSETFROMCENTERBOTTOM", isEditable);
                        if (GUIKwyttoCommons.AddFloatField(tabAreaSize.x, "K45_WTS_BOXMESH_DEPTH_BACK", item.BackgroundMeshSettings.FrameMeshSettings.BackDepth, out var newVal, isEditable, 0))
                        {
                            item.BackgroundMeshSettings.FrameMeshSettings.BackDepth = newVal;
                            changedFrame |= true;
                        }

                        if (GUIKwyttoCommons.AddFloatField(tabAreaSize.x, "K45_WTS_BOXMESH_DEPTH_FRONT", item.BackgroundMeshSettings.FrameMeshSettings.FrontDepth, out newVal, isEditable, 0))
                        {
                            item.BackgroundMeshSettings.FrameMeshSettings.FrontDepth = newVal;
                            changedFrame |= true;
                        }
                        if (GUIKwyttoCommons.AddFloatField(tabAreaSize.x, "K45_WTS_TEXT_CONTAINERFRONTBORDERTHICKNESS", item.BackgroundMeshSettings.FrameMeshSettings.FrontBorderThickness, out newVal, isEditable, 0, Mathf.Min(item.BackgroundMeshSettings.Size.X, item.BackgroundMeshSettings.Size.Y) / 2))
                        {
                    ...
                    if (usingFrame)
                    {
                        GUIKwyttoCommons.AddToggle("K45_WTS_TEXT_CONTAINERUSEVEHICLECOLOR", ref item.BackgroundMeshSettings.FrameMeshSettings.m_inheritColor, isEditable);
                        if (!item.BackgroundMeshSettings.FrameMeshSettings.m_inheritColor)
                        {
                            GUIKwyttoCommons.AddColorPicker("K45_WTS_BOXMESH_OUTERCOLOR", m_picker, ref item.BackgroundMeshSettings.FrameMeshSettings.m_cachedOutsideColor, isEditable);
                        }
                        GUIKwyttoCommons.AddColorPicker("K45_WTS_TEXT_CONTAINERGLASSCOLOR", m_picker, ref item.BackgroundMeshSettings.FrameMeshSettings.m_cachedGlassColor, isEditable);
                        ...
                        if (GUIKwyttoCommons.AddSlider(tabAreaSize.x, "K45_WTS_TEXT_CONTAINERGLASSTRANSPARENCY", item.BackgroundMeshSettings.FrameMeshSettings.GlassTransparency, out newVal, 0, 1, isEditable))
                        {
